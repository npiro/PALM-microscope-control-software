...
% Description:
%   progressbar(hObjects,handles,fractiondone,position) provides an indication of the progress of
...
%   progressbar(hObjects,handles,fractiondone,position)
%   progressbar(hObjects,handles)               % Initialize/reset
%   progressbar(hObjects,handles,0)            % Initialize/reset
%   progressbar(hObjects,handles,0,4)          % Initialize/reset and specify position
%   progressbar(hObjects,handles,0,[0.2 0.7])  % Initialize/reset and specify position
%   progressbar(hObjects,handles,0.5)          % Update
%   progressbar(hObjects,handles,1)            % Close
%
% Demo:
%   n = 1000;
...
% Check if hObject and handles has been passed
if nargin < 2
    error('You at least need to pass hObjects and handles arguments')
end
...
try
    % Access progfig, progpatch and progaxes to see if they exist ('try' will fail if it doesn't)
    
    progaxes = handles.progaxes;
    progpatch = handles.progpatch;
    progfig = handles.progfig;
    firstIteration = handles.firstIteration;
...
% Save handles to GUI handle holder
handles.progfig = progfig;
handles.progaxes = progaxes;
handles.progpatch = progpatch;
handles.firstIteration = firstIteration;
