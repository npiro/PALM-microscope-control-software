function aberrationSimu
figure(1000)

range = 256;
x = 1:range;

inputPSF = generateGaussianPSF(range,[range/2 range/2 range/2],[range/2 range/2 range/2]);
...
set( theFigure,'units','normalized',...
    'toolbar','figure',...
    'position',[0.01 0.01 0.88 0.90],...
    'color',[1 1 1],...
    'name','Aberrated PSF simulation',...
    'CloseRequestFcn' , @closeInterface);

axes_1 = axes('position',[0.75 0.95 0.23 0.23]);
...
forceSlider_listener = addlistener(force_slider,'Action',@chooseAberrationFunction);
    ...
    function chooseAberrationFunction(obj,event)
        ...
        cla(axes_1)
        pcolor(axes_1,x,x,mask), shading interp
    end
