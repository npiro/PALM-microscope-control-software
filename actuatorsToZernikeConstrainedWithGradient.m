...
ValidInfluenceMatrixGradient = DerivInfluenceMatrix(idx2,:)*5e6;
...
for m=1:NumZernikeModes
    ...
    [AmplitudesForEachZernikeMode(:,m),resnorm(m),residual(:,m),exitflag(m),output(m),lambda(m)]=lsqlin(ValidInfluenceMatrixGradient,ThisZernikeModeGradient,[],[],[],[],0*ones(32,1),1*ones(32,1),0.5*ones(32,1),opt);
    [AmplitudesForEachZernikeModeNeg(:,m),resnormNeg(m),residualNeg(:,m),exitflagNeg(m),outputNeg(m),lambdaNeg(m)]=lsqlin(ValidInfluenceMatrixGradient,ThisZernikeModeGradientNeg,[],[],[],[],0*ones(32,1),1*ones(32,1),0.5*ones(32,1),opt);
